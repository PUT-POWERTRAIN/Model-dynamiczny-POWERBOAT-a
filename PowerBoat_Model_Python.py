...
X_u, Y_v, N_r = -50.0, -100.0, -400.0  # N_r = -400.0 (Nasz wirtualny statecznik!)
...
# ==========================================
dt = 0.1
T_max = 30.0  # Wydłużamy do 30 sekund!
time_steps = np.arange(0, T_max, dt)

states_sim = np.zeros((len(time_steps), 6))
current_state = np.zeros(6) 

# NOWOŚĆ: Tablice na historię wejść sterujących (żeby je potem narysować)
...
for i in range(len(time_steps)):
    ...
    # --- ZADAWANIE WEJŚĆ (Twój zmodyfikowany test łosia) ---
    if t < 5.0:
        T_cmd = 0.0
        alpha_cmd = 0.0
    elif 5.0 <= t < 10.0:
        # Pchamy do przodu i skręcamy
        T_cmd = 150.0
        alpha_cmd = np.radians(-15.0) 
    elif 10.0 <= t < 15.0:
        # Gasimy silnik na prostych kołach
        T_cmd = 0.0
        alpha_cmd = 0.0
    else:
        # Odpalamy silnik po dryfowaniu
        T_cmd = 150.0
        alpha_cmd = 0.0
    ...
    # Zapisujemy wartości z tego kroku do tablic historycznych!
    T_hist[i] = T
    alpha_hist[i] = alpha
...
u_hist = states_sim[:, 3]
v_hist = states_sim[:, 4]  # Prędkość poprzeczna

# Konfiguracja głównego okna
fig = plt.figure(figsize=(15, 8))
# Dzielimy ekran: 4 wiersze, 2 kolumny. Prawa kolumna będzie szersza (ratio 1.5)
gs = gridspec.GridSpec(4, 2, width_ratios=[1, 1.5]) 

# --- LEWA STRONA (4 Wykresy Telemetrii) ---
# Oś X wszystkich wykresów będzie połączona (sharex)
...
# Rysujemy kąt w stopniach, bo człowiekowi łatwiej się to czyta niż radiany
ax_alpha.plot(time_steps, np.degrees(alpha_hist), 'm-', lw=2) 
ax_alpha.set_ylabel('Kąt α [°]')
ax_alpha.set_xlabel('Czas symulacji [s]')
ax_alpha.grid(True)

# Tworzymy poruszające się pionowe linie na wykresach telemetrii
...
ax_map = fig.add_subplot(gs[:, 1]) # gs[:, 1] oznacza "zajmij wszystkie 4 wiersze w 2. kolumnie"
...
boat_shape, = ax_map.plot([], [], 'r-', linewidth=3, label='Dziób Trimarana')
ax_map.legend()
...
fig.tight_layout() # Układa wykresy, żeby napisy na siebie nie wchodziły
